...
const Skills = () => {
  ...
  // change to your deployed API endpoint when not local

  // Fetch skills
  const fetchSkills = async () => {
    try {
      const response = await axios.get(`${API_BASE_URL}/skills`);
      setSkills(response.data.skills || []); // backend sends { skills: [...] }
    } catch (error) {
  ...
  const updateSkill = async () => {
    try {
      await axios.put(`${API_BASE_URL}/skills/${editSkill._id}`, editSkill);
      setEditSkill(null);
      fetchSkills();
    } catch (error) {
...
//   const API_BASE_URL = "http://localhost:3001/portfolio/api"; // for JavaScript ExpressJS
//   // const API_BASE_URL = "http://localhost:8000/portfolio/api"; // for FastAPI
//   // const API_BASE_URL = "csp-portfolio-springboot-svc.csp.svc.cluster.local:8080/portfolio/api"; // for Java
//   // const API_BASE_URL = "https://dev.cloudstones.org/portfolio/api";
